...
TddmCAData::TddmCAData(const CSInitData& data) :
...
void CustomTddmCANavMsg::buildNavMsg(int64_t elapsedTime, uint32_t prn, const CSConstellation& /*data*/)
{
...
CustomTddmCA::CustomTddmCA(const CSInitData& data) : ICustomSignal(data), m_data(data), m_msg(m_data), m_code(m_data)
{
}
...
ICustomSignalNavMsg* CustomTddmCA::getNavMsg()
{
  return &m_msg;
}

ICustomSignalCode* CustomTddmCA::getCode(const char* name)
{
...
DEFINE_CUSTOM_SIGNAL(CustomTddmCA)
