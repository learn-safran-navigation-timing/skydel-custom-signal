...
QzssCAData::QzssCAData(const CSInitData& data) :
...
void CustomQzssCANavMsg::buildNavMsg(int64_t elapsedTime, uint32_t prn, const CSConstellation& /*data*/)
{
...
CustomQzssCA::CustomQzssCA(const CSInitData& data) : ICustomSignal(data), m_data(data), m_msg(m_data), m_code(m_data)
{
}
...
ICustomSignalNavMsg* CustomQzssCA::getNavMsg()
{
  return &m_msg;
}

ICustomSignalCode* CustomQzssCA::getCode(const char* name)
{
...
DEFINE_CUSTOM_SIGNAL(CustomQzssCA)
